...
const
  fccBc7* = make_fourcc('D','X','1','0').DdsFourCC
...
proc init*(self: var DdsReader; f: File): bool =
  ...
  # now
  var supported = false
  for code in [fccDxt1, fccDxt2, fccDxt3, fccDxt4, fccDxt5, fccBc5, fccBc7]:
...
proc loadDDS*(path: string, image: var Image): bool =
  ...
  image.width = ddsReader.base_width()
  image.height = ddsReader.base_height()
  image.numMipmaps = ddsReader.total_mipmaps()
  image.numChannels = ddsReader.channel_count()
  
  if ddsReader.total_mipmaps == 0:
    assert ddsReader.read(f, image.data)
    assert not ddsReader.skip(f)
  else:
    for m in 0 ..< ddsReader.total_mipmaps:
      assert ddsReader.read(f, bytesRead)
      image.data.add(bytesRead)
    assert not ddsReader.skip(f)
...
proc stbir_resize_uint8*(
  input_pixels : ptr uint8, 
  input_w , input_h , input_stride_in_bytes: cint,
  output_pixels : ptr uint8, output_w, output_h, output_stride_in_bytes,
  num_channels: cint): cint {.importc, noconv.}
