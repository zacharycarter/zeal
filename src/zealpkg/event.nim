...
proc globalRegister*(eventKind: EventKind; handlerProc: HandlerProc;
    userArg: pointer; simMask: int32) =
  registerHandler(
    key(globalId, eventKind),
    HandlerDesc(
      kind: hkEngine,
...
proc globalNotify*(eventKind: EventKind; eventArg: pointer;
...
proc serviceQueue*() =
  ...
  handleEvent(Event(kind: ekUpdateEnd, arg: nil, source: esEngine,
      receiverId: globalId))
