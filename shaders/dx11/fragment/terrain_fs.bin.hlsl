...
{
return bgfxTexture2DArray(s_texColor, float3(uv, 3.0));
}
...
float4 bilinear_interp_vec4
(
float4 q11, float4 q12, float4 q21, float4 q22,
float x1, float x2,
float y1, float y2,
float x, float y
)
...
void main( float4 gl_FragCoord : SV_POSITION , nointerpolation int4 v_adjacentMatIndices : TEXCOORD4 , nointerpolation int v_blendMode : TEXCOORD3 , nointerpolation int v_materialID : TEXCOORD2 , float3 v_normal : NORMAL , float2 v_texcoord0 : TEXCOORD0 , float3 v_worldPos : TEXCOORD1 , out float4 bgfx_FragData0 : SV_TARGET0 )
{
...
break;
default:
tex_color = float4(1.0, 0.0, 1.0, 1.0);
return;
}
if (tex_color.a == 0.0)
discard;
float height = v_worldPos.y / 4;
float3 ambient = (float(0.7) + height * 0.03) * ambient_color;
float3 light_dir = normalize(light_pos - v_worldPos);
float diff = max(dot(v_normal, light_dir), 0.0);
float3 diffuse = light_color * (diff * float3(0.9, 0.9, 0.9));
float3 view_dir = normalize(view_pos - v_worldPos);
float3 reflect_dir = reflect(-light_dir, v_normal);
float spec = pow(max(dot(view_dir, reflect_dir), 0.0), 2);
float3 specular = 0.5 * light_color * (spec * float3(0.1, 0.1, 0.1));
bgfx_FragData0 = float4( (ambient + diffuse) * tex_color.xyz, 1.0);
}
